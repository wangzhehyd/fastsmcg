...
class geom_confs(Dataset):
    ...
    def featurize_mol(self, mol_dic):
        ...
        if mol_dic.get('uniqueconfs'):
            if mol_dic.get('uniqueconfs') <= self.max_confs:
                # use all confs
                conf_ids = np.arange(mol_dic.get('uniqueconfs'))
            else:
                # filter the most probable 'conf_per_mol' confs
                all_weights = np.array([_.get('boltzmannweight', -1.) for _ in mol_dic.get('conformers')])
                descend_conf_id = (-all_weights).argsort()
                conf_ids = descend_conf_id[:self.max_confs]
        else:
            return None
        ...
        #for conf in confs:
        try:
            [confs[i] for i in conf_ids]
        except:
            return None

        for conf in [confs[i] for i in conf_ids]:
            mol = conf['rd_mol']
