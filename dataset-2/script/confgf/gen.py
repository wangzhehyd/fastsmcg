...
if __name__ == '__main__':
    ...
    #                                        num_repeat=50, keep_traj=False,
    #                                        out_path=config.test.output_path)
    if args.smiles is not None:
        inputs = pd.read_csv(args.smiles)
        mol_index = 1
        for smiles, num_confs in tqdm(inputs.values):
            if int(num_confs) < 1000:
                num_confs = int(num_confs)*2
            else:
                num_confs = int(num_confs)
            #if os.path.exists(f'./test/confgf_mol_{mol_index}_conf_{num_confs}.pkl'):
            #    break
            solver.generate_samples_from_smiles(smiles, args.generator, num_repeat=num_confs, keep_traj=False, out_path=f'./test/confgf_mol_{mol_index}_conf_{num_confs}.pkl')
            mol_index += 1
